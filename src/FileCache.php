...
class FileCache {
	...
	public function get( string $file, callable $callback, $key = false ) {
		...
		if ( $key === false ) {
			return \is_file( $file_path )
				...
				? $this->get_data( $file_path )
				: $this->set( $callback, false, $file_path );
		}

		return \is_file( $file_path )
			// Cache key is set, if value is not cached then we're going to create a file and save the value
			...
			? $this->get_key_data( $file_path, $key, $callback )
			: $this->set( $callback, $key, $file_path );
	}
	 ...
	 * @since 0.1.0
	 */
	public function get_data( string $file_path ) {
	 ...
	 * @since 0.1.0
	 */
	protected function get_key_data( string $file_path, string $key, callable $callback ) {
		$file_data = $this->get_data( $file_path );
		...
		unset( $this->object_cache[ $file_path ] );

		// Renew our cached file.
		return $this->set( $file_data, false, $file_path )[ $key ];
	}
	 ...
	 * @since 0.1.0
	 */
	protected function set( $value, $key, string $file_path ) {
		...
		$file_data = $key === false
			...
			: [ $key => $this->get_value( $value ) ];

		\file_put_contents( $file_path, $this->render_php( $file_data ) );
		\chmod( $file_path, 0777 );

		$this->object_cache[ $file_path ] = $file_data;
	 ...
	 * @return false|mixed
	 */
	protected function get_value( $cache_value ) {
	 ...
	 * @since 0.1.0
	 */
	public function delete( $file = false ): array {
		if ( $file === false ) {
			return [];
		}

		$file = trailingslashit( $this->storage_path ) . $file;
		$deleted = [];

		// If path is a directory, then delete all the files inside the
		// dir and then the dir itself.
		if ( \is_dir( $file ) ) {
			foreach ( $this->get_path_names( $file . '/*' ) as $file_path ) {
